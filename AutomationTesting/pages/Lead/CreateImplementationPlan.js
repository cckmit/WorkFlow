...
class CreateImplementationPlan extends BasePage {
    ...
    csrSelect(csr_obj) {
        ...
        setTimeout(() => {
            element(by.css('[class="select2-search__field"]')).sendKeys(csr_obj);
            browser.waitForAngular();
            browser.sleep(3000);
            element(by.css('span[class="select2-container select2-container--default select2-container--open"] span[class*="select2-dropdown select2-dropdown--below"] span[class="select2-results"] ul[class="select2-results__options"] li[class*="select2-results__option--highlighted"]')).click()

        }, this.timeout.m);
    }
    approvalManagerSelect(manager_obj) {
        ...
        element.all(by.css('md-input-container[class*="multipleSelect2"] select[ng-model="vm.impPlan.devManager"] option[value="' + manager_obj + '"]')).click()

        /* var elm = element.all(by.css('.multipleSelect2')).click().then(function (ele) {
            ele.element(by.css('[class="select2-search__field"]')).sendKeys('aravinth');
            browser.sleep(3000)
            element(by.css('span[class="select2-container select2-container--default select2-container--open"] span[class*="select2-dropdown select2-dropdown--below"] span[class="select2-results"] ul[class="select2-results__options"] li[class*="select2-results__option--highlighted"]')).click()
        }).last() */
        ...
        /* 
        element(by.css('[ng-model="vm.impPlan.devManager"]')).element(by.css('[class="select2-search__field"]')).sendKeys('aravinth');
        browser.sleep(3000)
        element(by.css('span[class="select2-container select2-container--default select2-container--open"] span[class*="select2-dropdown select2-dropdown--below"] span[class="select2-results"] ul[class="select2-results__options"] li[class*="select2-results__option--highlighted"]')).click()
     */
    }

    planDescription(desc_obj) {
        this.enterText(this.descriptionPlan, desc_obj);
    }

    leadComments(comment_obj) {
        this.enterText(this.leadComment, comment_obj);
    }
    ...
    loadCategorySelect(loadcategory, sysid, i) {
        var self = this
        ...
        //     })
        // })
        var cat = element.all(by.css('[name="loadCategories"]')).get(i).$('[aria-label="' + sysid + '_' + loadcategory + '"]');
