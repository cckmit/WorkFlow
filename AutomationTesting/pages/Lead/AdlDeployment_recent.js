...
class AdlDeployment extends BasePage {
    ...
    deactivateCheckbox(activationstatus, capturedid, i, cbk) {
    ...
    statusWait(capturedid, cbk) {
        browser.wait(this.isNotVisible($('' + capturedid + '_NULL_0')), this.timeout.full);
        browser.waitForAngular();
        element(by.css('[aria-label="' + capturedid + '_SUCCESS_0"]')).isPresent().then(function (isPresent) {
            if (isPresent) {
                findid.capture.adlactivationstatus = isPresent
    ...
    deactivateAndDeleteCheckbox(activationstatus, capturedid, i, cbk) {
        var deactivateAndDelete = element(by.css('[aria-label="deactivateandDelete_' + capturedid + '_0"]'))
    ...
    activateCheckbox(activationstatus, capturedid, i, cbk) {
        var activate = element(by.css('[aria-label="activate_' + capturedid + '_0"]'))
  ...
  */
    loadAndActivateCheckbox(capturedid) {
        var loadAndActivate = element(by.css('[aria-label="loadandActivate_' + capturedid + '_0"]'))
    ...
    addTargetSystemSelect(capturedid) {
        findid.capture.adlactivationstatus = true
    ...
    targetSystemSelect(capturedid, target_obj) {
        var selectSystem = element(by.css('[aria-label="TargetSystem_' + capturedid + '_0z"]'))
        // var systemid = selectSystem.$('[value="' + sysid + '"]')
        var self = this
        var index
        $$('[aria-label="TargetSystem_' + capturedid + '_0"] option').map(function (el, index) {
            el.getText().then(function (txt) {
                // console.log(txt)
                if (txt.match(new RegExp(target_obj, 'gi')) != null) {
                    // console.log("Target System " + target_obj + " in index" + index)
                    var target = element.all(by.css('[aria-label="TargetSystem_' + capturedid + '_0"]')).all(by.tagName('option')).get(index)
    ...
    vparsIdSelect(capturedid, vpar_obj) {
        var index
        var self = this
        $$('[aria-label="testSystem_' + capturedid + '_0"] option').map(function (el, index) {
            el.getText().then(function (txt) {
                if (txt.match(new RegExp(vpar_obj, 'gi')) != null) {
                    // console.log("index" + index + "systemname :" + vpar_obj)
                    var vparsel = element.all(by.css('[aria-label="testSystem_' + capturedid + '_0"]')).all(by.tagName('option')).get(index)
