 ...
 * @author USER
 */
public class StageBuildActivityMessage extends ActivityLogMessage {
    ...
    public StageBuildActivityMessage(ImpPlan impPlan, Implementation implementation) {
	super(impPlan, implementation);
    }

    public StageBuildActivityMessage(ImpPlan impPlan, Implementation implementation, IBeans... beans) {
	...
	if (status.equals("initiated")) {
	    Message = "{0} {1} has {2} the staging build for the implementation plan {3}, system {4}";
	} else if (system == null) {
	    Message = "{0} {1} has {2} the staging build for the implementation plan {3}";
	} else {
	    Message = "{0} {1} has triggered the staging build for the implementation plan {3} for the system {4}. Status : {2}";
	}
