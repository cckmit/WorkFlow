dashboard.controller("qaMyTaskCtrl", function ($rootScope, $scope, $state, $location, Toaster, WFLogger, apiService,
    ...
    $scope.isTSSDeployed = function (planObj) {
        var isTSSD = false
        if (planObj.extraParams) {
            _.each(planObj.extraParams.qaTemplate, function (qatemplateObj) {
                _.each(qatemplateObj.prevDeploymentActions, function (prevDeploymentObj) {
                    if (planObj.planStatus == "APPROVED" && prevDeploymentObj.disableLoadActionDueToTSSDeploy && prevDeploymentObj.status == "ACTIVATED") {
                        isTSSD = true;
                    }
                })
            })
    ...
    $scope.applyPass_Fail = function (planObj, systemId, vparId, type, all, ev) {
        ...
        if (type == "PASS") {
            var loadedAction = _.filter(populatedActions, function (actionObj) {
                // (actionObj.status == "DELETED" && actionObj.vparId.name == "LTCUST") ||  (
                // == populatedActions.length
                return actionObj.status != "ACTIVATED" && (actionObj.vparId.name == "LTCUST" || actionObj.vparId.name == "CAP" || actionObj.vparId.name == "CGR" || actionObj.vparId.name == "CRE");
            })
            if (loadedAction && loadedAction.length && $scope.qaScreenType == "FUNCTIONAL") {
                // <System – LTCUST or LTQA>
                var confirm = $mdDialog.confirm()
                    .title('Are you sure?')
                    .textContent(paramObj.planId + " is not yet deployed to WSP : LTCUST. Are you sure you would like to mark it pass/fail? If you will mark it pass then plan will move for regression testing and no further actions will be performed")
                    .ariaLabel('reject')
                    .targetEvent(ev)
                    .ok('YES')
                    .cancel('NO');
